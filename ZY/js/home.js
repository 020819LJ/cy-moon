...
(function() {
    ...
    window.openApp = function(app) {
        const appActions = {
            ...
            // [已迁移] 'music' 入口已移至聊天页面底部工具栏耳机按钮，不再通过首页图标打开
        };
    ...
    async function loadSavedSettings() {
        ...
        if (savedOrder) {
            try {
                ...
                // 再将默认顺序中新增的项追加到最后
                defaultAppOrder.forEach(function(app) {
                    if (!seen.has(app)) {
                        merged.push(app);
                        seen.add(app);
                    }
                ...
                // ★ 层级修复：强制聊天图标始终排第一位
                var chatIdx = merged.indexOf('chat');
                if (chatIdx > 0) {
                    merged.splice(chatIdx, 1);
                    merged.unshift('chat');
                }
                appOrder = merged;
                // 检查是否有新增项被合并进来，或者chat位置被修正，更新 localStorage
                if (merged.length > saved.length || saved.indexOf('chat') !== 0) {
                    saveAppOrder();
                }
                reorderAppItems();
            } catch(e) {}
